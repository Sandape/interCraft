...
- 不修改产品代码、CI Workflow 语法、Ruleset 本身、客户端配置文件
- 不部署、不合并、不关闭 Issue、不代替真人 Reviewer
...
**Highest Risk Class**: R1。本需求治理文档和配置变更，不直接处理用户数据或财务操作；Agent 自动化（Phase 8）涉及 Issue 评论触发的写仓库操作，但使用最小权限 bot 身份。

| Governed operation / effect | Risk class | Actor / target / trust boundary | Required authorization and evidence |
|---|---|---|---|
| 治理文档与 SOP 创建/修改 | R0 | 治理负责人 / specs/ + docs/engineering/ | Issue dispatch + PR |
| 客户端适配文件创建/修改 | R1 | 治理负责人 / AGENTS.md, CLAUDE.md, .cursor/rules/ | Issue dispatch + PR + CODEOWNERS |
| 自动化 bot 开 PR | R1 | GitHub App / 仓库 | Issue dispatch + PR Gate |
| Issue Forms / PR Template 创建 | R0 | 治理负责人 / .github/ | Issue dispatch + PR |
...
- **FR-014**: Dispatch 状态机 MUST 满足：Inbox / Needs Clarification 可无 Driver；In Progress / In Review 必须有一个当前 Dispatch。
...
- **Dispatch Envelope**: dispatch_id, driver, base_sha, spec_task_id, ac_hash, allowed_paths, governance_version, created_at, state (active | superseded | expired)
...
- 本需求编号为 **REQ-064**：已验证为下一个可用的 SpecKit 编号。
...
- HTML 截图 walkthrough 作为验收素材由 Codex 截取和保存。