...
| 044 | Admin Console Redesign | active / Phase 1 (IA shell) | [spec.md](./044-admin-console-redesign/spec.md) | REQ-044 supersedes specs/035-admin-dashboard-mvp and specs/039-log-center-full. Phase 1 ships the 8-workspace IA shell + role-aware nav (FR-001~FR-006) + saved-view type stubs. Phase 2+ lands Command Center decision-queue content / AI ops / incidents / governance etc. |

> **Supersession note**: REQ-044 supersedes 035 (Admin Dashboard MVP) and 039 (Log Center Full). Both prior specs now carry `Status: Superseded` in their own frontmatter. Specs 035 and 039 are removed from the Active and Done tables below — only their supersession marker files remain as historical evidence.
