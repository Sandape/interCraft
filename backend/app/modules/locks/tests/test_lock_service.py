...
@pytest.fixture
def mock_redis():
    with patch("app.modules.locks.redis_store.get_redis") as m:
...
class TestLockServiceAcquire:
    ...
    async def test_acquire_409_different_user(self, mock_redis, mock_ws, mock_audit):
        ...
        from app.modules.locks.redis_store import RedisStore
        from app.modules.locks.service import LockService
        from app.modules.locks.schemas import AcquireInput

        store = RedisStore()
        resource_id = str(uuid4())
        # Simulate User A already holding the lock
        await store.acquire(
            "resume_branch",
            resource_id,
            {"user_id": str(uuid4()), "device_id": "dev-a"},
        )
        # Now User B tries
        ...
        assert "409" in str(exc_info.value) or "locked" in str(exc_info.value).lower()

    async def test_acquire_409_same_user_different_device(self, mock_redis, mock_ws, mock_audit):
        """Same user, different device — should get already_held_by_you."""
        from app.modules.locks.service import LockService
        from app.modules.locks.schemas import AcquireInput
...
class TestLockServiceRelease:
    async def test_release_success(self, mock_redis, mock_ws, mock_audit):
        ...
        svc = LockService()
        result = await svc.release(lock_id=str(uuid4()), user_id="user-1")
        assert result.lock_id is not None
    ...
    async def test_release_403_not_owner(self, mock_redis, mock_ws, mock_audit):
        ...
        svc = LockService()
        with pytest.raises(Exception) as exc_info:
            await svc.release(lock_id=str(uuid4()), user_id="user-b")
        assert "403" in str(exc_info.value) or "not yours" in str(exc_info.value).lower()
...
class TestLockServiceHeartbeat:
    async def test_heartbeat_success(self, mock_redis, mock_ws, mock_audit):
        ...
        svc = LockService()
        result = await svc.heartbeat(lock_id=str(uuid4()), user_id="user-1")
