...
| ResumeEditor | ✅ 通过 | 真实API | 分支创建、版本管理、信息面板正常。409 lock冲突预期行为 |
...
- **原因**: 早期开发时使用mock数据，BASE路径未添加 `/api/v1/` 前缀。Vite proxy配置为 `/api` → `http://127.0.0.1:8000`，所以正确的URL应该是 `/api/v1/...`
...
- **症状**: 所有DeepSeek LLM调用在30s超时+3次重试后失败 (90s max), 使用fallback:
  ...
  - 报告: fallback计算，无AI总结
- **原因**: 
  1. `DEEPSEEK_BASE_URL=https://api.deepseek.com/v1` — OpenAI SDK追加路径后产生双 `/v1` (/v1/v1/chat/completions)
...
- **验证**: curl测试两次API调用成功 (简单调用4s, 面试问题生成10s), 返回中文React面试题
...
- `dashboard-test.png` — Dashboard (mock数据可见)
