// REQ-034 US5 — VolunteerSectionList (left-rail items list + add-button).
//
// Renders the volunteer section's `items[]` as a sortable list of rows.
// Each row uses the shared SectionItem wrapper (US3 AC-01, R7).
//
// `data-dnd-context="volunteer"` for cross-section isolation
// (AC-04b: 11 list coexistence).
//
// VolunteerItem fields: `organization / location / period / website{...} / description`.
// (NO `position`, NO `summary` — R3 fix. field is `description`.)
...
import type { VolunteerItem, ResumeDataV2 } from "../../schema/data";
...
function freshItem(): VolunteerItem {
  return {
    id: newId(),
    hidden: false,
    organization: "",
...
function deepCloneItem(src: VolunteerItem): VolunteerItem {
  return {
    id: newId(),
    hidden: src.hidden,
...
export interface VolunteerSectionListProps {
  /** Section id, typically "volunteer". */
  sectionId?: string;
}

export function VolunteerSectionList({
  sectionId = "volunteer",
}: VolunteerSectionListProps): JSX.Element {
  ...
  const handleAdd = () => {
    ...
    setDataMut((draft) => {
      ...
      const arr = sec.items as unknown as VolunteerItem[];
  ...
  return (
    <div
      data-testid="volunteer-section-list"
      data-dnd-context="volunteer"
    ...
    >
      <div className="flex items-center justify-between">
        ...
        >
          Volunteer items ({items.length})
        </span>
        <button
          type="button"
          aria-label="添加 Volunteer 条目"
          onClick={handleAdd}
          data-testid="volunteer-add-item"
      ...
      {items.length === 0 && (
        <div
          ...
          data-testid="volunteer-section-list-empty"
        >
          暂无条目,点击 + Add 新增
        </div>
      )}
      ...
      >
        ...
        >
          <ul className="space-y-1" data-testid="volunteer-section-list-items">
            {items.map((item) => {
              const subtitle = item.period || item.location || "—";
              return (
                <SectionItem
                  key={item.id}
                  id={item.id}
                  ...
                  subtitle={subtitle}
                  titleTestId={`volunteer-organization-display-${item.id}`}
                  onEdit={handleEdit}
