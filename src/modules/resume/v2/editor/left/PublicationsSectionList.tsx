// REQ-034 US5 — PublicationsSectionList (left-rail items list + add-button).
//
// Renders the publications section's `items[]` as a sortable list of rows.
// Each row uses the shared SectionItem wrapper (US3 AC-01, R7).
//
// `data-dnd-context="publications"` for cross-section isolation
// (AC-04b: 11 list coexistence).
//
// PublicationItem fields: `title / publisher / date / website{...} / description`.
// (Schema uses `date` not `releaseDate` — R2 fix.)
...
import type { PublicationItem, ResumeDataV2 } from "../../schema/data";
...
function freshItem(): PublicationItem {
  return {
    id: newId(),
    hidden: false,
    title: "",
...
function deepCloneItem(src: PublicationItem): PublicationItem {
  return {
    id: newId(),
    hidden: src.hidden,
    title: src.title,
...
export interface PublicationsSectionListProps {
  /** Section id, typically "publications". */
  sectionId?: string;
}

export function PublicationsSectionList({
  sectionId = "publications",
}: PublicationsSectionListProps): JSX.Element {
  ...
  const handleAdd = () => {
    ...
    setDataMut((draft) => {
      ...
      const arr = sec.items as unknown as PublicationItem[];
  ...
  return (
    <div
      data-testid="publications-section-list"
      data-dnd-context="publications"
    ...
    >
      <div className="flex items-center justify-between">
        ...
        >
          Publications items ({items.length})
        </span>
        <button
          type="button"
          aria-label="添加 Publications 条目"
          onClick={handleAdd}
          data-testid="publications-add-item"
      ...
      {items.length === 0 && (
        <div
          ...
          data-testid="publications-section-list-empty"
        >
          暂无条目,点击 + Add 新增
        </div>
      )}
      ...
      >
        ...
        >
          <ul className="space-y-1" data-testid="publications-section-list-items">
            {items.map((item) => {
              const subtitle = item.publisher || item.date || "—";
              return (
                <SectionItem
                  key={item.id}
                  id={item.id}
                  ...
                  title={item.title}
                  subtitle={subtitle}
                  titleTestId={`publications-title-display-${item.id}`}
                  onEdit={handleEdit}
