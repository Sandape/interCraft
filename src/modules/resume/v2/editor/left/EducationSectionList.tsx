// REQ-034 US3 — EducationSectionList (left-rail items list + add-button).
//
// Renders the education section's `items[]` as a sortable list of rows.
// Each row uses the shared SectionItem wrapper (US3 AC-01, R7).
//
// Behaviour:
//   - add → push a fresh empty EducationItem, openDialog(education.update)
//   - edit → openDialog(education.update, itemId)
//   - duplicate → setDataMut pushes deep-copy with fresh id (no dialog)
//   - delete → setDataMut splices by id
//   - drag-reorder → dnd-kit, with `data-dnd-context="education"` for
//     cross-section isolation (AC-17b)
//
...
import type { EducationItem, ResumeDataV2 } from "../../schema/data";
...
function freshItem(): EducationItem {
  return {
    id: newId(),
    hidden: false,
    school: "",
...
function deepCloneItem(src: EducationItem): EducationItem {
  return {
    id: newId(),
    hidden: src.hidden,
    school: src.school,
    ...
    courses: src.courses.slice(),
  };
}

// ── component ──────────────────────────────────────────────────────────────

export interface EducationSectionListProps {
  /** Section id, typically "education". */
  sectionId?: string;
}

export function EducationSectionList({
  sectionId = "education",
}: EducationSectionListProps): JSX.Element {
  const items = useResumeV2Store(
    (s) => (s.data.sections[sectionId as keyof ResumeDataV2["sections"]] as { items: EducationItem[] }).items,
  );
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    ...
    // AC-17b: short-circuit if over container is a different dnd-context.
  ...
  return (
    <div
      data-testid="education-section-list"
    ...
    >
      <div className="flex items-center justify-between">
        <span id={titleId} className="text-[10px] font-semibold uppercase tracking-wide text-ink-3">
          Education items ({items.length})
        </span>
        <button
          type="button"
          aria-label="添加 Education 条目"
          onClick={handleAdd}
          data-testid="education-add-item"
      ...
      {items.length === 0 && (
        <div className="rounded border border-dashed border-surface-border p-2 text-[10px] text-ink-3" data-testid="education-section-list-empty">
          暂无条目,点击 + Add 新增
        </div>
      )}
      <DndContext
      ...
      >
        ...
        >
          <ul className="space-y-1" data-testid="education-section-list-items">
            {items.map((item) => {
              const subtitle = item.degree || item.period || "—";
              return (
                <SectionItem
                  key={item.id}
                  id={item.id}
                  ...
                  title={item.school}
                  subtitle={subtitle}
                  titleTestId={`${sectionId}-school-display-${item.id}`}
                  onEdit={handleEdit}
