// REQ-034 US5 — ReferencesSectionList (left-rail items list + add-button).
//
// Renders the references section's `items[]` as a sortable list of rows.
// Each row uses the shared SectionItem wrapper (US3 AC-01, R7).
//
// `data-dnd-context="references"` for cross-section isolation
// (AC-04b: 11 list coexistence).
//
// ReferenceItem fields: `name / position / website{...} / phone / description`.
// (NO `email` — R4 fix. Schema has `position` and `website{...}`.)
...
import type { ReferenceItem, ResumeDataV2 } from "../../schema/data";
...
function freshItem(): ReferenceItem {
  return {
    id: newId(),
    hidden: false,
    name: "",
...
function deepCloneItem(src: ReferenceItem): ReferenceItem {
  return {
    id: newId(),
    hidden: src.hidden,
    name: src.name,
...
export interface ReferencesSectionListProps {
  /** Section id, typically "references". */
  sectionId?: string;
}

export function ReferencesSectionList({
  sectionId = "references",
}: ReferencesSectionListProps): JSX.Element {
  ...
  const handleAdd = () => {
    ...
    setDataMut((draft) => {
      ...
      const arr = sec.items as unknown as ReferenceItem[];
  ...
  return (
    <div
      data-testid="references-section-list"
      data-dnd-context="references"
    ...
    >
      <div className="flex items-center justify-between">
        ...
        >
          References items ({items.length})
        </span>
        <button
          type="button"
          aria-label="添加 References 条目"
          onClick={handleAdd}
          data-testid="references-add-item"
      ...
      {items.length === 0 && (
        <div
          ...
          data-testid="references-section-list-empty"
        >
          暂无条目,点击 + Add 新增
        </div>
      )}
      ...
      >
        ...
        >
          <ul className="space-y-1" data-testid="references-section-list-items">
            {items.map((item) => {
              const subtitle = item.position || item.phone || "—";
              return (
                <SectionItem
                  key={item.id}
                  id={item.id}
                  ...
                  title={item.name}
                  subtitle={subtitle}
                  titleTestId={`references-name-display-${item.id}`}
                  onEdit={handleEdit}
