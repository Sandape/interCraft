// REQ-034 US3 — ProjectsSectionList (left-rail items list + add-button).
//
// Renders the projects section's `items[]` as a sortable list of rows.
// Each row uses the shared SectionItem wrapper (US3 AC-01, R7).
//
// Behaviour mirrors EducationSectionList, with `data-dnd-context="projects"`
...
import type { ProjectItem, ResumeDataV2 } from "../../schema/data";
...
function freshItem(): ProjectItem {
  return {
    id: newId(),
    hidden: false,
    name: "",
...
function deepCloneItem(src: ProjectItem): ProjectItem {
  return {
    id: newId(),
    hidden: src.hidden,
    name: src.name,
    ...
    highlights: src.highlights.slice(),
  };
}

// ── component ──────────────────────────────────────────────────────────────

export interface ProjectsSectionListProps {
  /** Section id, typically "projects". */
  sectionId?: string;
}

export function ProjectsSectionList({
  sectionId = "projects",
}: ProjectsSectionListProps): JSX.Element {
  const items = useResumeV2Store(
    (s) => (s.data.sections[sectionId as keyof ResumeDataV2["sections"]] as { items: ProjectItem[] }).items,
  );
  ...
  return (
    <div
      data-testid="projects-section-list"
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          aria-label="添加 Projects 条目"
          onClick={handleAdd}
          data-testid="projects-add-item"
      ...
      {items.length === 0 && (
        <div className="rounded border border-dashed border-surface-border p-2 text-[10px] text-ink-3" data-testid="projects-section-list-empty">
          暂无条目,点击 + Add 新增
        </div>
      )}
      <DndContext
      ...
      >
        ...
        >
          <ul className="space-y-1" data-testid="projects-section-list-items">
            {items.map((item) => {
              const subtitle = item.period || "—";
              return (
                <SectionItem
                  key={item.id}
                  id={item.id}
