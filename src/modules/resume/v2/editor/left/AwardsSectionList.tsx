// REQ-034 US5 — AwardsSectionList (left-rail items list + add-button).
//
// Renders the awards section's `items[]` as a sortable list of rows.
// Each row uses the shared SectionItem wrapper (US3 AC-01, R7).
//
// `data-dnd-context="awards"` for cross-section isolation
// (AC-04b: 11 list coexistence).
//
// AwardItem fields: `title / awarder / date / website{url,label,inlineLink} / description`.
...
import type { AwardItem, ResumeDataV2 } from "../../schema/data";
...
function deepCloneItem(src: AwardItem): AwardItem {
  return {
    id: newId(),
    hidden: src.hidden,
    title: src.title,
...
export interface AwardsSectionListProps {
  /** Section id, typically "awards". */
  sectionId?: string;
}

export function AwardsSectionList({
  sectionId = "awards",
}: AwardsSectionListProps): JSX.Element {
  ...
  const handleAdd = () => {
    ...
    setDataMut((draft) => {
      ...
      const arr = sec.items as unknown as AwardItem[];
  ...
  return (
    <div
      data-testid="awards-section-list"
      data-dnd-context="awards"
    ...
    >
      <div className="flex items-center justify-between">
        ...
        >
          Awards items ({items.length})
        </span>
        <button
          type="button"
          aria-label="添加 Awards 条目"
          onClick={handleAdd}
          data-testid="awards-add-item"
      ...
      {items.length === 0 && (
        <div
          ...
          data-testid="awards-section-list-empty"
        >
          暂无条目,点击 + Add 新增
        </div>
      )}
      ...
      >
        ...
        >
          <ul className="space-y-1" data-testid="awards-section-list-items">
            {items.map((item) => {
              const subtitle = item.awarder || item.date || "—";
              return (
                <SectionItem
                  key={item.id}
                  id={item.id}
                  ...
                  title={item.title}
                  subtitle={subtitle}
                  titleTestId={`awards-title-display-${item.id}`}
                  onEdit={handleEdit}
