/** GeneralCoach page — M19 conversational AI coach. */
...
export default function GeneralCoach() {
  ...
  const { loading, messages, detectedIntent, redirectTo, error, start, sendMessage, close, reset } = useGeneralCoach()
  ...
  return (
    <div className="h-full flex flex-col max-w-4xl mx-auto">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full text-center" data-testid="coach-empty-state">
            ...
            <p className="text-sm text-ink-3 max-w-md">
              我可以帮你准备面试、优化简历、提供职业发展建议，或回答技术问题。
            </p>
          </div>
        )}
        ...
        {detectedIntent && (
          <div className="text-2xs text-ink-3 text-right" data-testid="coach-intent">
            意图：{detectedIntent}
          </div>
        )}
...
function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    <div className={`flex items-start gap-2 ${isUser ? 'flex-row-reverse' : ''}`}>
      {isUser ? (
        <User className="h-5 w-5 text-ink-3 mt-1 flex-shrink-0" />
      ) : (
        <Bot className="h-5 w-5 text-brand-500 mt-1 flex-shrink-0" />
      )}
      <div
