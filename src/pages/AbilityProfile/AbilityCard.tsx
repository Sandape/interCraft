...
export default function AbilityCard({ dimension: d, onEdit, readOnly }: Props) {
  const trendIcon = d.trend === 'up' ? <TrendingUp className="h-3.5 w-3.5" /> :
    d.trend === 'down' ? <TrendingDown className="h-3.5 w-3.5" /> :
    <Minus className="h-3.5 w-3.5" />

  const trendColor = d.trend === 'up' ? 'text-emerald-500' :
    d.trend === 'down' ? 'text-red-500' :
    'text-ink-3'

  return (
    <div className="flex items-center justify-between p-3 rounded-lg border border-surface-border dark:border-dark-surface-border hover:border-brand-500/30 transition-colors">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-ink-1">{d.label_zh}</span>
          {d.self_assessed_score !== null && d.self_assessed_score !== d.actual_score && (
        ...
        <div className="flex items-center gap-3 mt-1">
          <span className="text-lg font-semibold text-ink-1 tabular-nums">{d.actual_score}</span>
